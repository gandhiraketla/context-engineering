...
class RAGDemo:
    ...
    def __init__(self):
        ...
        if not all([openai_api_key, pinecone_api_key, groq_api_key]):
            missing = []
        ...
        # Small model for pruning/summarization - Llama 3.3 70B via Groq
        self.pruning_llm = ChatOpenAI(
        ...
        # Large model for final answers
        self.answering_llm = ChatOpenAI(model="gpt-4o", temperature=0.1)
...
"""
            ...
            judge_result = self.answering_llm.invoke(judge_prompt).content
    ...
    def run_comparison(self, query: str, return_results: bool = False):
        ...
        # 1. Plain RAG - using OpenAI
        console.print("🤖 Running Plain RAG with OpenAI...", style="yellow")
        plain_answer = self.answering_llm.invoke(
            f"Based on the following context, please answer the question.\n\nContext:\n{original_context}\n\nQuestion: {query}\n\nAnswer:"
        ).content
        
        # 2. LLM-based Pruning - using Llama 3.3 70B for pruning
        console.print("📝 Running Llama 3.3 70B Pruning...", style="yellow")
        prompt_pruned_context = self.pruning_llm.invoke(
        ...
        if not return_results:
            console.print(Panel(prompt_pruned_context[:1000] + "..." if len(prompt_pruned_context) > 1000 else prompt_pruned_context,
                              title="📝 DeepSeek Pruned Context", border_style="blue"))
        
        # Then use OpenAI for final answer
        console.print("🤖 Generating answer with OpenAI...", style="yellow")
        pruned_answer = self.answering_llm.invoke(
        ...
        console.print(Panel(plain_answer, title="🤖 Plain RAG Answer (OpenAI)", border_style="white"))
        console.print(Panel(pruned_answer, title="📝 Pruned Answer (Llama 3.3 70B→OpenAI)", border_style="blue"))
...
def main():
    ...
    try:
        ...
        console.print("GROQ_API_KEY=your-key", style="yellow")
        console.print("LANGSMITH_API_KEY=your-key (optional)", style="yellow")
